...
export class RefreshPoller {
  ...
  async run(feedIds?: string[]): Promise<RefreshStatus> {
    await this.client.startRefresh(feedIds);
    if (this.active) return this.active;
    const run = this.poll().finally(() => { this.active = null; });
    this.active = run;
    return run;
  }
  ...
  sync(fromStart = false): Promise<RefreshStatus> {
    ...
    const job = this.drain().finally(() => { this.syncing = null; });
    this.syncing = job;
    return job;
  }
  ...
  private async drain(): Promise<RefreshStatus> {
    while (true) {
      ...
      this.reset = false;
      const status = await this.client.refreshStatus(this.cursor, this.feedCursor);
